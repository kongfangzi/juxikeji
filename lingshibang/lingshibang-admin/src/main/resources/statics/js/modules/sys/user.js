...
var vm = new Vue({
    ...
    methods: {
        ...
        saveOrUpdate: function () {
            ...
            if (vm.user.password == '' || vm.user.password.length < 8){
                alert("密码至少为8位");
                return;
            }
            var passwordFormat = /^(?=.*[0-9])(?=.*[A-Z])(?=.*[a-z])(?=.*[^a-zA-Z0-9]).{8,}$/;
            if (!passwordFormat.test(vm.user.password)) {
                alert("密码必须包含数字、大写字母、小写字母、特殊字字符");
                return;
            }
    		...
    		if(vm.user.roleIdList == null || vm.user.roleIdList == ''){
        		alert("角色不能为空");
        		return;
    		}  		
            $.ajax({
